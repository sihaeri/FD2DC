...
module LAMMPS
   ...
   public :: lammps_open, lammps_open_no_mpi, lammps_close, lammps_file, &
      ...
      lammps_scatter_atoms,lammps_get_ntypes, lammps_gather_atoms_image
   ...
   interface
      ...
      subroutine lammps_actual_gather_atoms_image(ptr,image) & 
      bind (C, name='lammps_gather_atoms_image')
         import :: C_ptr
         type (C_ptr), value :: ptr
         type (C_ptr), value :: image
      end subroutine lammps_actual_gather_atoms_image
   end interface
...
!-------------------------------------------------------------------------2}}}
   ...
   subroutine lammps_gather_atoms_ia (ptr, name, count, data)
      ...
      if(allocated(data))THEN;deallocate( data );end if
   ...
   subroutine lammps_gather_atoms_image(ptr, image)
      type(C_ptr), intent(in) :: ptr
      integer, dimension(:), allocatable, intent(out) :: image
      type (C_ptr) :: Cdata
      integer (C_int), dimension(:), pointer :: Fdata
      integer (C_int) :: natoms
      integer (C_int),PARAMETER :: count = 3

      natoms = lammps_get_natoms (ptr)
      
      allocate ( Fdata(count*natoms) )
      if(allocated(image))THEN;deallocate( image );end if
      allocate ( image(count*natoms) )
      
      Cdata = C_loc (Fdata(1))
      call lammps_actual_gather_atoms_image (ptr, Cdata)
      image = Fdata
      deallocate (Fdata)
   end subroutine lammps_gather_atoms_image
