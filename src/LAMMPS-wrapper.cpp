...
/*Decode image flags in 3*natom array image*/
void lammps_gather_atoms_image(void* ptr, void* image)
{
  ...
  lmp->memory->create(dummy,natoms,"lammps_wrapper/gather_atoms_image");

  lammps_gather_atoms(ptr, name, type, count, static_cast<void*>(dummy));
  
  for(int i=0; i<natoms; i++)
  { 
    static_cast<int*>(image)[i*icount]   = (dummy[i] & IMGMASK) - IMGMAX;
    static_cast<int*>(image)[i*icount+1] = (dummy[i] >> IMGBITS & IMGMASK) - IMGMAX;
    static_cast<int*>(image)[i*icount+2] = (dummy[i] >> IMG2BITS) - IMGMAX;
  }
  lmp->memory->destroy(dummy);
  return;
}
