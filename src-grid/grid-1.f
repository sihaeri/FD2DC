...
C
    ...
    1 FORMAT(A12)
      IF(FILIN.NE.'*') THEN
        ...
        ITYP=0
      ELSE
        OPEN (UNIT=1,FILE='grid.inp',IOSTAT = ierr)
        REWIND 1
        ITYP=1
      ENDIF
...
C-----------------------------------------------------------
      ...
      READ(5,*) LPLOT,KG1,NGR
      IF(ITYP.EQ.1) WRITE(1,*) LPLOT,KG1,NGR,'  LPLOT,KG1,NGR '
C
      CALL GRIDN(ITYP)
C-------------------------------------------------
...
C####################################################################
     ...
     *       XG(1000),YG(1000),XALL(2000),YALL(2000)
      COMMON /PARAM/ NI,NJ,NIM,NJM,NIGR(8),NJGR(8),
     *       IGR(8),JGR(8),IJGR(8),KG1,NGR,LPLOT
      DIMENSION XX(2000),SPX(2000)
      REAL KSIBAR,A,B
C     FI(A,B,G,N)=A*G**N-B*G+B-A
C
C------------------------------------------------------
...
C---------------------------------------------------------------
      DO LL=1,2
        ...
        READ(5,*) NSUB
        IF(ITYP.EQ.1) WRITE(1,*) NSUB,'   NSUB '
        NP=1
C
        DO L=1,NSUB
    5     CONTINUE
...
C-------------------------------------------
          IF (L.EQ.1) THEN
            ...
            READ(5,*) XXS,XXE,DELS1,N 
            IF(ITYP.EQ.1) WRITE(1,*) XXS,XXE,N,'   XXS,XXE,N '
            PRINT *, ' ENTER> EXPANSION FACTOR (ZERO OR SPEC. VALUE)  '
            READ(5,*) G
            IF(ITYP.EQ.1) WRITE(1,*) G,'   G '
            IF (G.EQ.0.) THEN
              PRINT *, ' REF. CV SIZE, POSITION (1 - BEG., 2 - END)  '
              READ(5,*) A,IP 
              IF(ITYP.EQ.1) WRITE(1,*) A,IP, '   A,IP '
            ENDIF
          ELSE
C----------------------------------------
...
C----------------------------------------
            ...
            READ(5,*) XXE,N
            IF(ITYP.EQ.1) WRITE(1,*) XXE,N,'   XXE,N '
            A=XX(NP)-XX(NP-1)
            IP=1
          ENDIF
C--------------------------------------------------------------------
...
!          ENDIF
          IF(L == 1)THEN
            DELSS = (XXE-XXS)/REAL(N)
            S0 = DELSS/DELS1
            B = CALC_INV_SINOVERA(S0)
            DO I=1,N           ! for the nodes of this spline
              KSIBAR = REAL(I-1)/REAL(N)
              SPX(I) = (XXE-XXS)*(1.0000 + 
     X              TANH(B/2.0000*(KSIBAR - 1.0000))/TANH(B/2.0000))
            ENDDO
          ELSEIF(L == 2)THEN
            DO I=1,N
              SPX(I) = I * DELS1
          ...
          ELSEIF(L == 3)THEN
            ...
            S0 = DELSS/DELS1
            B = CALC_INV_SINOVERA(S0)
            DO I=1,N-1           ! for the nodes of this spline
              KSIBAR = REAL(I)/REAL(N-1)  
              SPX(I) = (XXE-XXS)*(1.0000 + 
     X          TANH(B/2.0000*(KSIBAR - 1.0000))/TANH(B/2.0000))
            ENDDO
          ELSE
            PRINT *,'This program is hacked for 3 subdomains.'
            PAUSE
          ENDIF
C----------------------------------------
...
C
      ...
      FUNCTION CALC_INV_SINOVERA(S0)

        REAL :: S0,CALC_INV_SINOVERA,S0BAR,V,W

        IF(S0 < 2.7829681)THEN 
          S0BAR = S0 - 1.0000000  
          CALC_INV_SINOVERA = SQRT(6.00*S0BAR)*(1.0000000 - 0.15*S0BAR
     X                      + 0.057321429*S0BAR**2 - 0.02490729*S0BAR**3
     X                  + 0.0077424461*S0BAR**4 - 0.0010794123*S0BAR**5)
        ELSEIF(S0 > 2.7829681)THEN
          V = LOG(S0)
          W = 1.0000000/S0 - 0.028527431
          CALC_INV_SINOVERA = V + (1.0000000 + 1.0000000/V)*LOG(2.0*V) -
     X                              0.02041793 + 0.24902722*W +
     X              1.9496443*W**2 - 2.6294547*W**3 + 8.56795911*W**4
  
        ENDIF
      END FUNCTION calc_inv_sinovera
C
C#######################################################
